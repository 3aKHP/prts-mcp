...
|------|------|------|
| Python | 1.7.2 | LTS |
| TypeScript | 1.7.2 | LTS |

- 当前 LTS 发布：32 个 MCP 工具（1.7.2）
- 本维护线最新发布：1.7.2 LTS
- 1.7.2 LTS 发布内容：敌方数据库上游形状兼容（战斗属性恢复）、双实现 JSON 占位与输入边界加固、排序/正则 parity、同步元数据互通与原子化、剧情 KeyError 消息清理、TS 会话空闲淘汰修复、TS 生产依赖安全更新；不改变 32 个工具或 transport。
...
| 版本 | 日期 | 亮点 |
|------|------|------|
| 1.7.2 | 2026-09-26 | LTS：敌方数据库兼容恢复战斗属性、同步互通与加固、会话空闲/输入边界/依赖安全修复 |
