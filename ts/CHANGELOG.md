...
- **Enemy cluster split (P3.A).** `data/enemy.ts` and `data/stageEnemy.ts` shed their pure logic into new 1:1 module pairs: `enemyStats.ts` (stat extraction + mergeDefined + the deduplicated formatting shims `pythonFloatString`/`formatNumber` — previously four scattered private copies), `enemyRender.ts` (shared handbook card + stats block with the TS-only `hasEnemyStatsContent` guard preserved), `levelParser.ts` (level JSON parsing; dedupes the two `parseLevel` copies), and `enemyDatabase.ts` gains `level0Index`. `stageEnemy.ts` consumes enemy/stage's exported accessors instead of parallel cache instances; its dataset shrinks to `enemy_appearance_index`. **User-visible (debug)**: the `/debug/cache` `stage_enemy` section drops the four duplicated keys.
