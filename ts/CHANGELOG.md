...
- **DatasetAccess contract (P1.B).** New `data/datasetAccess.ts` (factory + named registry + `excelStore`/`levelsStore`), `data/gamedataAttrs.ts` (`mValue` deduplicated from enemy/stageEnemy, now with overloads preserving enemy's typed call sites), and `data/messages.ts` (canonical missing-data / bounds / regex message builders). All seven gamedata-family modules migrate onto the contract, retiring the hand-rolled `let` + `checkActivationChange()` + `CacheMetrics` scaffold (~200 lines; the tri-state `TableCache` post-guards were dead code — a throwing load always left the variable null, i.e. retry semantics — and are deleted); cache keys, named clear functions, and the payload-or-string surface are unchanged. `cacheStats.ts` now aggregates via the dataset registry (nine-key output and order unchanged). **User-visible**: item/stage missing-data messages now use the canonical family; enemy's divergent `无效的 limit/offset 参数：…` validation strings unify to the canonical `limit 必须 >= 1。` family (also enforcing limit ≤ 200).
