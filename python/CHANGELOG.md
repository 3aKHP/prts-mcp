...
- Activation-lock wait timeouts now raise a named `_ActivationLockTimeout` (a `TimeoutError` subclass, so existing `except TimeoutError` consumers keep working), re-exported from `prts_mcp.data.sync`; parity with the new TS `ActivationLockTimeoutError`. The error message is unchanged.
- Large-shard image downloads (`_download_large`) now use a **30-min total deadline per mirror attempt** (default raised from 300 s, with a `time.monotonic()` check in the chunk loop), matching the TS twin's per-attempt `AbortSignal.timeout`. Previously Python had no wall-clock cap at all, so a connect or inter-chunk stall of >300 s failed fast while slow-but-progressing downloads ran unbounded. Note the tightening: an attempt whose total duration exceeds 30 min now fails (and cascades to the next mirror with a fresh budget); the worst-case wait per dead mirror rises from 300 s to 1800 s. Only shard/delta downloads are affected — discovery (10 s), release assets (120 s) and lock waits (120 s) are unchanged.
