...
async def search_prts(query: str, limit: int = 5) -> str:
    ...
    Args:
        query: 搜索关键词，支持中文，如"罗德岛"、"整合运动"。
        limit: 返回结果数量上限，默认 5，最大建议不超过 10。
    """
...
async def read_prts_page(page_title: str) -> str:
    ...
    Args:
        page_title: 词条标题，需与维基页面标题完全一致，如"阿米娅"、"整合运动"。
                    建议通过 search_prts 获取准确标题后再传入。
    """
...
async def get_operator_archives(operator_name: str) -> str:
    ...
    Args:
        operator_name: 干员的游戏内中文名，如"阿米娅"、"能天使"。
    """
...
def read_story(story_key: str, include_narration: bool = True) -> str:
    ...
    Args:
        ...
        include_narration: 是否包含旁白和场景描述，默认 True。设为 False 可只保留对话台词。
    """
    from prts_mcp.config import Config
    cfg = Config.load()
    try:
