...
| 全量 E2E 真机测试流程（高风险改动强烈建议，维护者当次裁量） | [`docs/dev/E2E.md`](docs/dev/E2E.md) |
...
11. **reopen 随回灌 PR 顺手做**：回灌 PR 开出前先向同一个 `release/vX.Y.Z` 追加一个 `chore(release): reopen vNext development` commit——bump 到下一目标版本 + 加回 `-dev` / `.dev0` 后缀，并重新打开空 `[Unreleased]` 段，让双轨 CR 覆盖追加后的 head（不开单独 chore PR；下一目标版本按 [`docs/dev/VERSIONING.md`](docs/dev/VERSIONING.md) 的回灌规则确定）
...
- re-review 与追问：仅在 Bot 结论对合并决策确有必要时才 `@khpilot`（或 App 提及形式 `@khpilot[bot]`），请求里给出新 head SHA 和验证结果，避免主执行 Agent 与 Bot 陷入循环
