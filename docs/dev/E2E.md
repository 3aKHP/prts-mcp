...
**例外通道**：对 Standard 及以下等级的单个改动，维护者可按改动范围与验证成本明确决定以面向改动范围的轻量化真机验证替代当次全量流程（先例：PR #194 会话管理改动）；PR 必须披露验证范围与未执行环节，未执行的全量 E2E 仍留在发布前检查单上。Huge PR 与 Release 不适用本例外。规则全文见 [`WORKFLOW.md`](WORKFLOW.md)。
